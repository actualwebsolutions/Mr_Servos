...
class UsuariosTest < Test::Unit::TestCase
  ...
  def test_should_not_rehash_password
    ...
    assert_equal usuarios(:quentin), Usuarios.authenticate('quentin2', 'test')
  end

  def test_should_authenticate_usuarios
    assert_equal usuarios(:quentin), Usuarios.authenticate('quentin', 'test')
  end
...
protected
  def create_usuarios(options = {})
    record = Usuarios.new({ :login => 'quire', :email => 'quire@example.com', :password => 'quire', :password_confirmation => 'quire' }.merge(options))
    record.save
    record
  end
end
